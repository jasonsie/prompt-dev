...
6. **Craft Triggering Examples**: Create 2-4 `<example>` blocks showing:
   ...
   - Assistant response demonstrates agent triggering naturally
...
3. **Select Configuration**:
   ...
   - **Advanced fields** (include only when appropriate):
     - `disallowedTools`: Tool denylist (inverse of tools)
     - `permissionMode`: default/acceptEdits/delegate/dontAsk/bypassPermissions/plan
     - `maxTurns`: Limit agentic turns to prevent runaway execution
     - `skills`: Preload specific skills for domain knowledge
     - `mcpServers`: Scope MCP servers to this agent only
     - `hooks`: Scoped lifecycle hooks (e.g., SubagentStop validation)
...
4. **Generate Agent File**: Use Write tool to create `agents/[identifier].md`:
   ...
   permissionMode: [mode]     # Optional, only if needed
   maxTurns: [number]         # Optional, only if needed
   skills: ["skill-name"]     # Optional, only if agent needs domain knowledge
   ---
