...
2. **Validate Structure**:
   ...
   - Optional fields: `version`, `context`, `agent`, `hooks`, `user-invocable`, `argument-hint`
   - Deprecated fields: `when_to_use` (use description only)
   - Body content exists and is substantial

   **Advanced frontmatter checks** (if present):
   - `context: fork` - Valid only with `fork` value
   - `agent` - Must reference existing agent when `context: fork` is set
   - `hooks` - Valid hook event configuration
   - `user-invocable: false` - Skill hidden from slash menu
   - `argument-hint` - Documents expected arguments when skill accepts them
